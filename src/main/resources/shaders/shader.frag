...
void main(){
    vec2 texCoord = parallaxMapping(outTexCoord, normalize(-vertexOutput.tangentVertexPos));
    if(texCoord.x > 1.0 || texCoord.y > 1.0 || texCoord.x < 0.0 || texCoord.y < 0.0) discard;
    setupColors(material,texCoord);
    vec3 newNormal = calcNormal(material, mvVertexNormal, texCoord, outModelViewMatrix);
